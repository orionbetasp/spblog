...
func SignupPost(c *gin.Context) {
	...
	util.Logger.Info("SignupPost", zap.Any("user", user))
	if len(user.Email) == 0 || len(user.Password) == 0 {
		res["message"] = "email or password cannot be null"
		return
	}
	user.Password = util.Md5(user.Email + user.Password)
	err = user.Insert()
	if err != nil {
...
func SigninPost(c *gin.Context) {
	...
	if err != nil || user.Password != util.Md5(username+password) {
		c.HTML(http.StatusOK, "auth/signin.html", gin.H{
	...
	_ = s.Save()
	util.Logger.Info("SigninPost", zap.Any("user", user))
	if user.IsAdmin {
...
func Oauth2Callback(c *gin.Context) {
	...
	util.Logger.Info("Oauth2Callback", zap.Any("userInfo", userInfo))
...
func exchangeTokenByCode(code string) (accessToken string, err error) {
	...
	transport = &oauth.Transport{Config: &oauth.Config{
		ClientId:     conf.Con.GetString("github.github_clientid"),
		ClientSecret: conf.Con.GetString("github.github_clientsecret"),
		RedirectURL:  conf.Con.GetString("github.github_redirecturl"),
		TokenURL:     conf.Con.GetString("github.github_tokenurl"),
		Scope:        conf.Con.GetString("github.github_scope"),
	}}
