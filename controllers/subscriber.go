...
func Subscribe(c *gin.Context) {
	...
	if len(mail) > 0 {
		...
		if err == nil {
			if !subscriber.VerifyState && util.GetCurrentTime().After(subscriber.OutTime) { //激活链接超时
...
func sendActiveEmail(subscriber *models.Subscriber) (err error) {
	...
	subscriber.OutTime = util.GetCurrentTime().Add(duration)
	subscriber.SecretKey = uuid
	signature := util.Md5(subscriber.Email + uuid + subscriber.OutTime.Format("20060102150405"))
	subscriber.Signature = signature
	err = sendMail(subscriber.Email, "[spblog]邮箱验证", fmt.Sprintf("%s/active?sid=%s", conf.Con.GetString("domain"), signature))
	if err != nil {
...
func ActiveSubscriber(c *gin.Context) {
	...
	if !util.GetCurrentTime().Before(subscriber.OutTime) {
		HandleMessage(c, "激活链接已过期，请重新获取！")
		return
	}
	subscriber.VerifyState = true
	subscriber.OutTime = util.GetCurrentTime()
	err = subscriber.Update()
	if err != nil {
...
func GetUnSubcribeUrl(subscriber *models.Subscriber) (string, error) {
	uuid := util.UUID()
	signature := util.Md5(subscriber.Email + uuid)
	subscriber.SecretKey = uuid
	subscriber.Signature = signature
	err := subscriber.Update()
	return fmt.Sprintf("%s/unsubscribe?sid=%s", conf.Con.GetString("domain"), signature), err
}
